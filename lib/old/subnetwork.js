function draw_subnetwork(el, nodes, edges, styling){
    ...
    // Set up scales
    var x = d3.scale.linear().range([0, drawWidth]),
    ...
    force.on("tick", function() {
        circle.attr("transform", function(d) {
            d.x = Math.max(r, Math.min(drawWidth - r, d.x));
    ...
    fig.append("rect")
        ...
        .attr("x", drawWidth-netLegendWidth)
        .style("fill", "#fff");
    ...
    var netLegend = fig.selectAll(".net-group")
        ...
        .append("g")
            .attr("transform", function(d, i){
                return "translate(" + (drawWidth-netLegendWidth) + "," + ((i+1)*netLegendBox) + ")";
            })
            .style("font-size", 12)
            .on("click", function(n){
